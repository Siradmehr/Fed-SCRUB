...
class LinearNeuralTangentKernel(nn.Linear):
    ...
    def __init__(self, in_features, out_features, bias=True, beta=np.sqrt(0.1), w_sig=np.sqrt(2.0)):
        self.beta = beta
        super(LinearNeuralTangentKernel, self).__init__(
...
class NTK_MLP(nn.Module):
    def __init__(self, num_classes=10, filters_percentage=1.0, beta=np.sqrt(0.1)):
        ...
        self.fc2 = LinearNeuralTangentKernel(
            self.n_wid, num_classes, beta=beta)
...
class MLP(nn.Module):
    ...
    def _make_layers(self):
        ...
        layer += [
            ...
            StandardLinearLayer(self.input_size, self.hidden_size),
            # Affine(self.hidden_size),
            nn.ReLU()]
        for i in range(self.num_layer - 2):
            layer += [StandardLinearLayer(self.hidden_size,
        ...
        layer += [
            ...
            StandardLinearLayer(self.hidden_size, self.num_classes)]
...
class ConvStandard(nn.Conv2d):
    ...
    def __init__(self, in_channels, out_channels, kernel_size=3, stride=1, padding=None, output_padding=0, w_sig=np.sqrt(1.0)):
        super(ConvStandard, self).__init__(
    ...
    def reset_parameters(self):
        torch.nn.init.normal_(self.weight, mean=0, std=self.w_sig /
...
class AllCNN(nn.Module):
    def __init__(self, filters_percentage=1.0, n_channels=3, num_classes=10, dropout=False, batch_norm=True):
        ...
        self.features = nn.Sequential(
            ...
            Conv(n_filter1, n_filter2, kernel_size=3,
                 stride=2, padding=1, batch_norm=batch_norm),
            nn.Dropout(inplace=True) if dropout else Identity(),
            Conv(n_filter2, n_filter2, kernel_size=3,
                 stride=1, batch_norm=batch_norm),
            Conv(n_filter2, n_filter2, kernel_size=3,
                 stride=1, batch_norm=batch_norm),
            Conv(n_filter2, n_filter2, kernel_size=3, stride=2,
                 padding=1, batch_norm=batch_norm),  # 14
            nn.Dropout(inplace=True) if dropout else Identity(),
            Conv(n_filter2, n_filter2, kernel_size=3,
                 stride=1, batch_norm=batch_norm),
            Conv(n_filter2, n_filter2, kernel_size=1,
                 stride=1, batch_norm=batch_norm),
            nn.AvgPool2d(8),
            Flatten(),
        )
...
class ConvNeuralTangentKernel(nn.Conv2d):
    ...
    def __init__(self, in_channels, out_channels, kernel_size=3, stride=1, padding=None, output_padding=0, w_sig=np.sqrt(1.0)):
        super(ConvNeuralTangentKernel, self).__init__(
            in_channels, out_channels, kernel_size)
        self.in_channels = in_channels
        self.out_channels = out_channels
        self.kernel_size = kernel_size
...
class ntk_AllCNN(nn.Module):
    def __init__(self, filters_percentage=1., n_channels=3, num_classes=10, dropout=False, batch_norm=True):
        ...
        n_filter2 = int(192 * filters_percentage)
        self.features = nn.Sequential(
            ntk_Conv(n_channels, n_filter1, kernel_size=3,
                     batch_norm=batch_norm),
            ntk_Conv(n_filter1, n_filter1, kernel_size=3,
                     batch_norm=batch_norm),
            ntk_Conv(n_filter1, n_filter2, kernel_size=3,
                     stride=2, padding=1, batch_norm=batch_norm),
            nn.Dropout(inplace=True) if dropout else Identity(),
            ntk_Conv(n_filter2, n_filter2, kernel_size=3,
                     stride=1, batch_norm=batch_norm),
            ntk_Conv(n_filter2, n_filter2, kernel_size=3,
                     stride=1, batch_norm=batch_norm),
            ntk_Conv(n_filter2, n_filter2, kernel_size=3, stride=2,
                     padding=1, batch_norm=batch_norm),  # 14
            nn.Dropout(inplace=True) if dropout else Identity(),
            ntk_Conv(n_filter2, n_filter2, kernel_size=3,
                     stride=1, batch_norm=batch_norm),
            ntk_Conv(n_filter2, n_filter2, kernel_size=1,
                     stride=1, batch_norm=batch_norm),
            nn.AvgPool2d(8),
            Flatten(),
        )
...
class ResNet18(nn.Module):
    def __init__(self, filters_percentage=1.0, n_channels=3, num_classes=10, block=_ResBlock, num_blocks=[2, 2, 2, 2],
        ...
        self.layer1 = self._make_layer(block, int(
            64 * filters_percentage), num_blocks[0], stride=1)
        self.layer2 = self._make_layer(block, int(
            128 * filters_percentage), num_blocks[1], stride=2)
        self.layer3 = self._make_layer(block, int(
            256 * filters_percentage), num_blocks[2], stride=2)
        self.layer4 = self._make_layer(block, int(
            512 * filters_percentage), num_blocks[3], stride=2)
...
class ConvImprovedStandard(nn.Conv2d):
    ...
    def __init__(self, in_channels, out_channels, kernel_size=3, stride=1, padding=(0, 0), output_padding=0, w_sig=np.sqrt(2.0), s=10000):
        super(ConvImprovedStandard, self).__init__(
            in_channels, out_channels, kernel_size)
        self.in_channels = in_channels
        self.out_channels = out_channels
        self.kernel_size = kernel_size
    ...
    def reset_parameters(self):
        torch.nn.init.normal_(self.weight, mean=0, std=1 /
                              np.sqrt(self.in_channels * np.prod(self.kernel_size)))
        if self.bias is not None:
            torch.nn.init.normal_(self.bias, mean=0, std=0)
...
class wide_basicIS(nn.Module):
    def __init__(self, in_planes, planes, dropout_rate, stride=1):
        ...
        self.bn1 = nn.BatchNorm2d(in_planes)
        self.conv1 = ConvImprovedStandard(
            in_planes, planes, kernel_size=3, padding=(1, 1))
        self.dropout = nn.Dropout(p=dropout_rate)
        self.bn2 = nn.BatchNorm2d(planes)
        self.conv2 = ConvImprovedStandard(
            planes, planes, kernel_size=3, stride=stride, padding=(1, 1))

        self.shortcut = nn.Sequential()
        if stride != 1 or in_planes != planes:
            self.shortcut = nn.Sequential(
                ConvImprovedStandard(
...
class Wide_ResNetIS(nn.Module):
    def __init__(self, depth=4, filters_percentage=1.0, widen_factor=1, dropout_rate=0.0, num_classes=10):
        ...
        self.layer1 = self._wide_layer(
            wide_basicIS, nStages[1], n, dropout_rate, stride=1)
        self.layer2 = self._wide_layer(
            wide_basicIS, nStages[2], n, dropout_rate, stride=2)
        self.layer3 = self._wide_layer(
            wide_basicIS, nStages[3], n, dropout_rate, stride=2)
        self.bn1 = nn.BatchNorm2d(nStages[3], momentum=0.9)
        self.linear = nn.Linear(nStages[3], num_classes)
...
class ConvNTK(nn.Conv2d):
    ...
    def __init__(self, in_channels, out_channels, kernel_size=3, stride=1, padding=(0, 0), output_padding=0, w_sig=np.sqrt(2.0)):
    ...
    def forward(self, input):
        return F.conv2d(input, self.w_sig * self.weight / np.sqrt(self.in_channels * np.prod(self.kernel_size)), self.bias, self.stride, self.padding)
...
class wide_basicNTK(nn.Module):
    def __init__(self, in_planes, planes, dropout_rate, stride=1):
        ...
        self.dropout = nn.Dropout(p=dropout_rate)
        self.bn2 = nn.BatchNorm2d(planes)
        self.conv2 = ConvNTK(planes, planes, kernel_size=3,
...
class Wide_ResNetNTK(nn.Module):
    def __init__(self, depth=4, filters_percentage=1.0, widen_factor=1, dropout_rate=0.0, num_classes=10):
        ...
        self.layer1 = self._wide_layer(
            wide_basicNTK, nStages[1], n, dropout_rate, stride=1)
        self.layer2 = self._wide_layer(
            wide_basicNTK, nStages[2], n, dropout_rate, stride=2)
        self.layer3 = self._wide_layer(
            wide_basicNTK, nStages[3], n, dropout_rate, stride=2)
        self.bn1 = nn.BatchNorm2d(nStages[3], momentum=0.9)
        self.linear = nn.Linear(nStages[3], num_classes)
